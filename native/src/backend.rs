...
const BACKEND_NAME: &str = "scraper-mcp-backend.exe";
const BACKEND_PORT: u16 = 10998;
const BACKEND_TAG: &str = "scraper-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "MCP_PORT";
const ENV_HOST: &str = "MCP_HOST";
const ENV_TAURI: &str = "SCRAPER_MCP_TAURI";
...
fn free_port(port: u16) -> bool {
    ...
    {
        let img_kill = format!(
            "Stop-Process -Name 'scraper-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'scraper_mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM scraper-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM scraper_mcp-native.exe /T 2>$null"
        );
    ...
    thread::spawn(move || {
        for attempt in 0..30 {
            ...
            match TcpStream::connect_timeout(&addr, Duration::from_secs(2)) {
                Ok(_) => {
                    log_line(&app_health, &format!("Backend health check PASSED (attempt {})", attempt + 1));
        ...
        log_line(&app_health, "Backend health check FAILED");
