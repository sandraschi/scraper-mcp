...
foreach ($cfg in $configs) {
    ...
    try {
        ...
        if ($json.mcpServers."scraper-mcp" -eq $null) {
            $json.mcpServers | Add-Member -Name "scraper-mcp" -Value @{ url = "http://127.0.0.1:10998/mcp" } -MemberType NoteProperty
            $json | ConvertTo-Json -Depth 10 | Set-Content $cfg.Path
            Write-Host "  Registered scraper-mcp in $($cfg.Name)" -ForegroundColor Green
            $updated++
        } else {
